...
//------------------------------------------------------------------------------
  ...
  h_N_PV2->TH1::SetDefaultSumw2();
  h_N_PV3 = CreateH1F ("h_N_PV3","h_N_PV3",50,0,50); 
  h_N_PV3->TH1::SetDefaultSumw2();
...
//------------------------------------------------------------------------------
  ...
  if ( T_Muon_Px->size() > 1 ) {  // require events with at least two muons 
	...
	if (MatchGenToReco(Mu1, Mu2, Signal)) { 
	  ...
	  if (passesWWSelection(Mu1, Mu2, 0)) {
	    ...
	    FillRelEff("WWlevel", 0, Mu1, factN, iVertex, newiVertex);
	    FillRelEff("WWlevel", 1, Mu2, factN, iVertex, newiVertex);
...
void muonAnalyzer::GetAllHWWMuons() {
  ...
  if ( _muonSize > 0 ) {  // asking for at least one muon in the event 
    ...
    for (unsigned int i = 0; i < _muonSize; i++) {
      ...
      isMuonID = passHWWMuCuts(i);
      
      // Define the muon ISO
      isMuonISO = isMuonID*passPFIso(i, "dBetaR03");
      ...
      //double PFRelIsoBeta = getPFRelIso(i, "dBetaR03");     
      //G_Muon_PFIsoBeta.push_back(PFRelISoBeta);
...
void muonAnalyzer::GetAllTightMuons() {
  ...
  if ( muonSize > 0 ) {  // asking for at least one muon in the event 
    ...
    for (unsigned int i = 0; i < muonSize; i++) {
      ...
      // Define the VBTF muon ID      
      isMuonID = passTightMuCuts(i, 0.01, 0.02, 0.1, 0, 5);
...
bool muonAnalyzer::passTightMuCuts(int iMu,
  ...
  if ( iVertex >= 0 && fabs(get_dz(iMu, iVertex)) < cutdz && T_Muon_fromPV->at(iMu) > 1) 
    muon_sel[8] = true;
...
bool muonAnalyzer::passHWWMuCuts(int iMu) {
  ...
  if ( iVertex >= 0 && fabs(get_dz(iMu, iVertex)) < 0.1) 
    muon_sel[7] = true;
...
void muonAnalyzer::SetGenInfo(TString Signal) {
  ...
  if ( Signal.Contains("Wjets"))
  {
    ...
    if ( G_isTauMu ) {
      G_GEN_PromptTauMuon_4vec.push_back(TLorentzVector(T_Gen_PromptTau_LepDec_Px->at(0),
  ...
  /*for (unsigned int i = 0; i < genPromptMuSize && genPromptMuSize > 0; i++) {
    
    //-->define the global 4D momentum for each muon. 
    G_GEN_PromptMuon_4vec.push_back(TLorentzVector(T_Gen_FinalMuon_Px->at(i), 
						   T_Gen_FinalMuon_Py->at(i),
						   T_Gen_FinalMuon_Pz->at(i), 
						   T_Gen_FinalMuon_Energy->at(i)));
						   }*/
...
bool muonAnalyzer::MatchGenToReco(int &mu1, int &mu2, TString signal) {
  ...
  if (signal.Contains("GGHWW") || signal.Contains("DY")) {
    ...
    if ( G_isMuMu || G_isTauMu || G_isTauTau) isMatched = true;
 ...
 void muonAnalyzer::FillRelEff(string levelCut, int indexMuon, int iMu, double weight, int _iVertex, int _newiVertex) {
  ...
  if ( levelCut == "Dilep") { 
    ...
    if(T_Muon_IsGlobalMuon->at(iMu) && T_Muon_IsPFMuon->at(iMu)) {
      ...
      h_Dilep_dz_BestTrack[indexMuon]->Fill(fabs(T_Muon_BestTrack_dz->at(iMu)));
	...
	if ( T_Muon_NValidHitsSATrk->at(iMu) > 0 ) {
	  ...
	  if ( T_Muon_NumOfMatchedStations->at(iMu) > 1 ) {
		...
		h_Dilep_dz_BestTrack_bf_dy[indexMuon]->Fill(fabs(T_Muon_BestTrack_dz->at(iMu)), weight);
		...
		if ( (G_Muon_4vec[indexMuon].Perp() < 20 &&  T_Muon_IPwrtAveBSInTrack->at(iMu)  < 0.01) ||
		  ...
		  if ( _dz != 999 && (_dz < 0.1) && (T_Muon_fromPV->at(iMu) > 1)) { 
...
void muonAnalyzer::Filldz2D(int iMu1, int iMu2, double weight, int _iVertex, int _newiVertex)
{
  ...
  double _dzBT1 = fabs(T_Muon_BestTrack_dz->at(iMu1));
  double _dzBT2 = fabs(T_Muon_BestTrack_dz->at(iMu2));
